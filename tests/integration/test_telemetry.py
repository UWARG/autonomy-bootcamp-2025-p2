...
TELEMETRY_TIMEOUT_S = TELEMETRY_PERIOD
...
def read_queue(
    ...
    while not controller.is_exit_requested() or not output_queue.queue.empty():
        try:
            ...
            continue
        main_logger.info(f"Telemetry output: {item}", True)
...
def main() -> int:
    ...
    args = {"controller": controller, "output_queue": output_queue}

    # Just set a timer to stop the worker after a while, since the worker infinite loops
    threading.Timer(TELEMETRY_PERIOD_S * NUM_TRIALS * 2 + NUM_FAILS, stop, (args,)).start()
