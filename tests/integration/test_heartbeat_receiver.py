...
def stop(
    ...
    state_queue.fill_and_drain_queue()


def read_queue(
    state_queue: queue_proxy_wrapper.QueueProxyWrapper,  # Add any necessary arguments
...
def main() -> int:
    ...
    # Create your queues
    state_queue = queue_proxy_wrapper.QueueProxyWrapper(mp_manager, 0)

    # Just set a timer to stop the worker after a while, since the worker infinite loops
    threading.Timer(
        HEARTBEAT_PERIOD * (NUM_TRIALS * 2 + DISCONNECT_THRESHOLD + NUM_DISCONNECTS + 2),
        stop,
        args=(controller, state_queue),
    ).start()

    # Read the main queue (worker outputs)
    threading.Thread(
        target=read_queue, args=(state_queue, controller, main_logger), daemon=True
    ).start()
