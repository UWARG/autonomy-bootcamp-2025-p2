...
def put_queue(
    ...
    path: list[telemetry.TelemetryData],  # Add any necessary arguments
) -> None:
    """
    Place mocked inputs into the input queue periodically with period TELEMETRY_PERIOD.
    """
    for i in path:
        input_queue.queue.put(i)
...
def main() -> int:
    ...
    # Create your queues
    input_queue = queue_proxy_wrapper.QueueProxyWrapper(mp_manager, 1)
    ...
    threading.Thread(target=read_queue, args=(main_logger, input_queue, controller)).start()
